...
library(xts)
xts <- xts(time.data[,-1] * 100, time.data[,1])
...
ggplot(chloro, aes(long, lat)) +
  geom_polygon(aes(group=group, fill=Positive * 100)) + 
  labs(fill = "% Positive") + 
  coord_quickmap() +
...
ggplot(chloro, aes(long, lat)) +
  geom_polygon(aes(group=group, fill=Negative * 100)) + 
  labs(fill = "% Negative") + 
  coord_quickmap() +
...
################################################################
chloro$Difference <- ((chloro$Negative - chloro$Positive) * 100)
ggplot(chloro, aes(long, lat)) +
...
plot((Positive * 100)~submission_score, data=submission.data, col='darkgreen', pch='.',
     main="Sentiment By Score on Submission",
     xlab="Submission Score", ylab="% Sentiment")
points((Negative * 100)~submission_score, data=submission.data, col='red', pch='.')
...
plot((Positive * 100)~comment_score, data=comment.data, col='darkgreen', pch='.',
     main="Sentiment By Score on Comments",
     xlab="Comment Score", ylab="% Sentiment")
points((Negative * 100)~comment_score, data=comment.data, col='red', pch='.')
